...
class BeastRepository(
    private val beastWebService: BeastWebService,
    private val coroutineDispatcher: CoroutineDispatcher,
    private val beastDao : BeastDao
) {
    suspend fun getBeasts() : Flow<DataResult<List<MonsterDetail>>> {
        return flow {
            emit(DataResult.Loading)
            try {
                val infoDao = beastDao.getMonsterDetails()
                if(infoDao.isNotEmpty()) {
                    emit(DataResult.Success(infoDao))
                } else {
                    ...
                    beastDao.insertAllMonsterDetails(beastDetails)
                }
