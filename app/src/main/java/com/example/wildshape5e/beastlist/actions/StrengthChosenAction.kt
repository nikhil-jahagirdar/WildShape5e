...
import com.example.wildshape5e.beastlist.updaters.StrengthUpdater

class StrengthChosenAction(private val newStrength : Int) : BaseAction<BeastListState> {
    override suspend fun performAction(
        currentState: BeastListState,
        sendUpdate: (BaseUpdater<BeastListState>) -> Unit,
        sendEvent: (BaseEvent) -> Unit
    ) {
        sendUpdate(StrengthUpdater(newStrength))
    }
}