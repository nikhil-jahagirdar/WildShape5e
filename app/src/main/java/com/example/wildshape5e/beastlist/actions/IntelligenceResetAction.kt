...
import com.example.wildshape5e.beastlist.IntelligenceFilter
import com.example.wildshape5e.beastlist.updaters.IntelligenceUpdater

class IntelligenceResetAction() : BaseAction<BeastListState> {
    override suspend fun performAction(
        currentState: BeastListState,
        sendUpdate: (BaseUpdater<BeastListState>) -> Unit,
        sendEvent: (BaseEvent) -> Unit
    ) {
        sendUpdate(IntelligenceUpdater(null))
    }
}