...
import com.example.wildshape5e.beastlist.updaters.RatingUpdater

class RatingChosenAction(private val newRating : Int) : BaseAction<BeastListState> {
    override suspend fun performAction(
        currentState: BeastListState,
        sendUpdate: (BaseUpdater<BeastListState>) -> Unit,
        sendEvent: (BaseEvent) -> Unit
    ) {
        sendUpdate(RatingUpdater(newRating))
    }
}